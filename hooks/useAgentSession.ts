...
export function useAgentSession(opts: UseAgentSessionOptions) {
  ...
  // Live bash/powershell tool results cached outside `messages` so they survive
  // an SSE reconnect while the run is still active; ChatWindow seeds its
  // toolResultsMap from here (mirrors upstream def1478).
  ...
  const handleAgentEvent = useCallback((event: AgentEventLike) => {
    switch (event.type) {
      ...
      case "tool_execution_update": {
        ...
        // Cache live shell output so an SSE reconnect doesn't lose it mid-run.
  ...
  return {
    // State
    loading, error, activeLeafId, messages, entryIds, historyCursor, hasEarlierMessages, streamState, activeToolResults,
