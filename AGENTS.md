...
- **Idle timeout**: 10 minutes by default; configurable via `PI_WEB_IDLE_TIMEOUT_MS` (ms; `0` disables idle eviction). Extensions can defer idle eviction through the session-liveness registry (`globalThis[Symbol.for("@agegr/pi-web/session-liveness/v1")]`, see `lib/session-liveness.ts`) — keep the registry key and contract identical to upstream so extensions target one protocol. Explicit shutdown and Stop-fallback cleanup always take precedence. Concurrent `startRpcSession()` calls share a single start Promise (`globalThis.__piStartLocks`)
...
### History Rendering Must Bypass SDK Context Filtering
`buildSessionContext` (`lib/session-reader.ts`) renders the raw active branch via `sliceActiveBranch` and converts entries itself (`entryToUiMessage`, which also renders compaction cards). Do not route windows through the SDK's `buildContextEntries`: that filter drops any page whose window does not contain the compaction's `firstKeptEntryId`, silently losing history. `/api/files` catch-all paths resolve through `filePathFromSegments` (`lib/file-paths.ts`), which maps a bare drive letter to the drive root; keep that normalization when touching file routes.
...
### Chat Appearance, Search and Other v0.9.0 Feature Ports
- **Chat width/font size** (`hooks/useChatAppearance.ts`): `--chat-content-max-width`/`--chat-content-font-size` CSS variables on `:root` drive the whole conversation column (ChatWindow, ChatInput, session info bar) via `var()` fallbacks — no hardcoded 820px may return. Font sizes inside the conversation derive from `--chat-font-size-offset` (scoped by the `chat-content` class on ChatWindow's root); the composer textarea and its highlight layer must share the same font-size variable or the transparent-text overlay misaligns. Preferences live in localStorage (`pi-chat-content-width`, `pi-chat-content-font-size`) and are independent of `--app-ui-scale`.
- **Session full-text search** (`lib/session-search.ts`, `/api/sessions/search`, `components/SessionSearch.tsx`): literal, case-insensitive scan of the most recent 500 session files with time/size/result budgets; the sidebar's existing search box doubles as the full-text search (hits replace the session list while a query is active). Jump targets flow AppShell → ChatWindow via `searchTarget`; the ChatWindow locate path may pull one extra 200-entry history page (`loadContext(..., { tail })`), so keep the `data-entry-id` message wrappers and `data-search-target` block markers intact.
- **Cross-window session-list sync**: `/api/sessions` and `/api/agent/running` expose `sessionListVersion` (from `getSessionListVersion()`); the sidebar's running-state poll reloads the list when the version moves. Reads must never bump the version (no refresh loop).
- **Plugin updates** (`lib/plugin-updates.ts`, `/api/plugins/check`): check-only route keeps CLI/network work server-side; `canCheckForUpdates` flags npm/git sources without pinned versions. Batch update reuses `POST /api/plugins { action: "update" }` with a project-trust guard when project-scope packages are configured. Uses the direct `semver` dependency (added with `@types/semver`).
- **Mermaid preview**: completed diagrams default to preview (`MermaidBlock` in MarkdownBody, preview gated on `!isStreaming`); inline previews are height-bounded in `.mermaid-block`; SVG download goes through XMLSerializer (`downloadMermaidSvg`) because Mermaid's HTML serialization leaves void tags unclosed.
- **File panel video preview**: `.webm` is video, not audio (`VIDEO_EXT_TO_MIME`); video streams through the Range-capable file route with a `VideoViewer` mirroring the local AudioViewer (no `watchEnabled` prop in this architecture).
- **Subagents — full series (Phases 1–5 complete)** (`lib/subagents.ts`, `lib/subagent-runtime.ts`, `lib/subagent-extension.ts`, `lib/subagent-input.ts`, `lib/subagent-prompt.ts`, `lib/subagent-settings.ts`, `lib/subagent-profile-precedence.ts`, `app/api/subagents/*`, `components/AgentsConfig.tsx`): pure application-layer implementation on pi SDK 0.85.1 (SDK has no native subagent support). Profiles are markdown+YAML frontmatter files in four scopes — `builtin` (general-purpose/explore/plan), `global` (`getAgentDir()/agents`), `workspace` (`<cwd>/.agents/agents`, read-only), `project` (`<cwd>/.pi/agents`); higher scopes shadow lower ones (`listSubagentProfiles` dedupes by name, `isSubagentProfileOverridden` flags shadowed sources). Writable scopes are global/project only; project writes are guarded by `isExistingPathWithinRoots` (realpath-based, from `lib/path-security.ts`). `lib/frontmatter.ts` is the shared generic frontmatter parser (js-yaml); `lib/atomic-file.ts` `writePrivateFileAtomicSync` persists profiles. `isWindowsAbsolutePath` lives in `lib/paths.ts` (file-access re-exports it) so `path-security.ts` stays dependency-clean; route handlers keep the `guardRequest` pattern (`isApiRequestAllowed` on every method, `hasJsonContentType` on writes).
  - **Runtime**: subagent sessions are created via `SessionManager.create(cwd, undefined, { parentSession: parent.sessionFile })` so the parent link rides the normal session-file header (same grouping semantics as forks/worktrees). Orchestration lives in `lib/subagent-runtime.ts` (concurrency slots, resource snapshots, background completion via `pi-web:subagent-notification` custom messages with `deliverAs: "followUp"`); `lib/rpc-manager.ts` keeps only thin wiring. Subagents share the parent's session registry, idle-timeout and running-SSE infrastructure — do not fork off separate lifecycle paths.
  - **Session data**: `pi-web:subagent` (task metadata) and `pi-web:subagent-result` (outcome) are plain `custom` entries in the subagent's own file; `entryToUiMessage` maps both to custom UI messages (unknown custom entry types stay dropped) so opening a subagent session shows its task and result. Relation info (`relation: { kind: "subagent", ... }` in `SessionInfo`) is read bounded (prefix + tail) by `readSessionRelationEntries`; deleting a parent cascades/reparents children.
  - **UI**: `AgentsConfig.tsx` is a `SettingsPane`-based manager page under Settings → Agents (`settings-nav` id `agents`, needs a workspace); the built-in subagents master toggle lives there and flags `reloadNeeded` when a live session needs `reload`. `CustomMessageView` in MessageView gives subagent custom messages friendly titles + status chips (customType strings are mirrored client-side, not imported — `lib/subagents.ts` is server-only). Subagent sessions appear in the sidebar's existing parentSession tree; no separate switcher panel was ported (deliberate scope decision). i18n uses the `agents.*` namespace (en/zh-CN only; no zh-TW).
  - **Concurrency & lifecycle**: runs per parent are FIFO-queued (`lib/subagent-queue.ts`, `maxConcurrent` default 10 / cap 32) instead of rejected; a `pi-web:subagent-status` custom entry records queued/running so a persisted run shows the right state. Finished subagent sessions can be resumed via the `Agent` tool's `resume` parameter (reopened through the shared registry and re-queued). `isolation: "worktree"` runs the subagent in a fresh `pi-web-agent-<uuid>` worktree created with `addWorktree` from `lib/worktree.ts` and removed after completion; a failed cleanup (e.g. dirty worktree) retains it and reports `worktreeCleanupError`. Extension tools can be scoped per-profile via `ext:<name>/<tool>` selectors (`selectSubagentExtensionTools`).
  - **Profile round-tripping**: profile files are shared with other runtimes, so `saveSubagentProfile` carries frontmatter keys this app does not own (`name`, `allowed_subagents`, `exclude_extensions`, `disallowed_tools`, …) through untouched and keeps `ext:` tool selectors; managed keys are exactly `description`, `display_name`, `tools`, `load_skills`, `load_extensions`, `enabled`, `inherit_context`, `run_in_background`, `model`, `thinking`, `max_turns`, `prompt_mode`, `color`, `isolation`, `persist_session`. The `skills`/`extensions` aliases pi-subagents reads are seeded on first save and kept in step while they stay booleans; a hand-authored whitelist such as `extensions: pi-advisor-flow` is never rewritten, and `load_skills`/`load_extensions` fall back to those aliases when absent. Malformed existing frontmatter is refused rather than overwritten.
