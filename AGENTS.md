...
- **Subagents — profile layer** (`lib/subagents.ts`, `lib/subagent-profile-precedence.ts`, `/api/subagents/profiles`): Phase 1 of the subagent series (runtime/UI phases pending). Profiles are markdown+YAML frontmatter files in four scopes — `builtin` (general-purpose/explore/plan), `global` (`getAgentDir()/agents`), `workspace` (`<cwd>/.agents/agents`, read-only), `project` (`<cwd>/.pi/agents`); higher scopes shadow lower ones (`listSubagentProfiles` dedupes by name, `isSubagentProfileOverridden` flags shadowed sources). Writable scopes are global/project only; project writes are guarded by `isExistingPathWithinRoots` (realpath-based, from `lib/path-security.ts`). `lib/frontmatter.ts` is the shared generic frontmatter parser (js-yaml); `lib/atomic-file.ts` `writePrivateFileAtomicSync` persists profiles. Session-entry metadata types live in `lib/subagents.ts` (`pi-web:subagent`, `pi-web:subagent-result` custom entries; `SubagentSessionStatus` in `lib/types.ts`) for the runtime phase. `isWindowsAbsolutePath` now lives in `lib/paths.ts` (file-access re-exports it) so `path-security.ts` stays dependency-clean; route handlers must keep the `guardRequest` pattern (`isApiRequestAllowed` on every method, `hasJsonContentType` on writes).
