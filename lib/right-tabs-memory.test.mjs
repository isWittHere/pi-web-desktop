...
const { loadRightTabs, saveRightTabs, clearRightTabs, sanitizeTabs } = await jiti.import("./right-tabs-memory.ts");
...
test("clears only the targeted workspace and drops the store when empty", () => {
  saveRightTabs("/repo-a", { tabs: [fileTab], activeTabId: null, open: false });
  saveRightTabs("/repo-b", { tabs: [], activeTabId: null, open: false });
  clearRightTabs("/repo-a");
  assert.equal(loadRightTabs("/repo-a"), null);
  assert.ok(loadRightTabs("/repo-b"));
  clearRightTabs("/repo-b");
  assert.equal(store.has("pi-web:right-tabs-by-workspace"), false);
});
