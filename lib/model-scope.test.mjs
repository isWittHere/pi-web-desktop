...
test("suppresses a leftover unmatched glob while other entries still match", async () => {
  // `retired/*` matches nothing but `anthropic/*` still yields a usable list.
  const mixed = await resolveVisibleModels(runtime, ["retired/*", "anthropic/*"]);
  assert.equal(mixed.scopedModels.length, 2);
  assert.equal(mixed.warnings.length, 0);
});

test("keeps a no-match warning when every enabled pattern misses", async () => {
  const missed = await resolveVisibleModels(runtime, ["retired/*"]);
  assert.equal(missed.scopedModels.length, 0);
  assert.equal(missed.warnings.length, 1);
});

test("keeps malformed pattern warnings regardless of other matches", async () => {
  // A glob with a thinking-level suffix that is invalid is not suppressible.
  const malformed = await resolveVisibleModels(runtime, ["anthropic/*:totally-invalid"]);
...
test("rejects an ambiguous bare exact model id before fuzzy matching", async () => {
  await assert.rejects(
    resolveVisibleModels(runtime, ["claude-sonnet"]),
    /Ambiguous enabledModels entry.*anthropic\/claude-sonnet.*gateway\/claude-sonnet.*provider\/modelId/,
  );
});

test("accepts provider-qualified and unique bare exact model ids", async () => {
  const qualified = await resolveVisibleModels(runtime, ["anthropic/claude-sonnet"]);
  assert.deepEqual(references(qualified), ["anthropic/claude-sonnet"]);

  const unique = await resolveVisibleModels(runtime, ["claude-opus"]);
  assert.deepEqual(references(unique), ["anthropic/claude-opus"]);
});

test("rejects an ambiguous bare exact model id with a thinking suffix", async () => {
  await assert.rejects(
    resolveVisibleModels(runtime, ["claude-sonnet:high"]),
