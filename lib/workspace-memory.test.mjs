...
test("duplicate folded keys resolve to the most recent write (no pinning)", () => {
  if (process.platform !== "win32") return; // Windows-only concern
  store.clear();
  // A legacy store may hold the same workspace under several spellings.
  // The lobby write (external detector path) lands first; a later pi-native
  // write must win even though the exact stored keys differ.
  setLastOpen("E:/dev/pi-web-main", { kind: "session", id: "s-old" });   // external case
  setLastOpen("E:\\Dev\\pi-web-main", { kind: "session", id: "s-new" }); // pi-native backslash
  assert.deepEqual(getLastOpen("E:/dev/pi-web-main"), { kind: "session", id: "s-new" });
  assert.deepEqual(getLastOpen("E:/Dev/pi-web-main"), { kind: "session", id: "s-new" });
  assert.deepEqual(getLastOpen("E:\\Dev\\pi-web-main"), { kind: "session", id: "s-new" });
  store.clear();
});

test("writes collapse folded-equivalent duplicate keys to one slot", () => {
  if (process.platform !== "win32") return; // Windows-only concern
  store.clear();
  // Simulate a legacy store with two spellings already persisted.
  store.set(
    "pi-web:last-open-by-workspace",
    JSON.stringify({
      "E:/dev/pi-web-main": { kind: "session", id: "s-a" },
      "E:/Dev/pi-web-main": { kind: "session", id: "s-b" },
    })
  );
  setLastOpen("E:\\Dev\\pi-web-main", { kind: "session", id: "s-c" });
  const raw = store.get("pi-web:last-open-by-workspace");
  const keys = Object.keys(JSON.parse(raw));
  const foldedKeys = keys.map((k) => k.replace(/\\/g, "/").toLowerCase());
  assert.equal(new Set(foldedKeys).size, keys.length, "no folded-equivalent duplicates left");
  assert.equal(keys.length, 1, "exactly one slot for the workspace");
  assert.deepEqual(getLastOpen("E:/dev/pi-web-main"), { kind: "session", id: "s-c" });
  store.clear();
});
