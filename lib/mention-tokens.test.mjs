...
// jiti resolves the subject's extensionless relative imports (comment-mentions)
// that plain node ESM cannot.
const jiti = createJiti(import.meta.url);

async function loadSubject() {
  return jiti.import("./mention-tokens.ts");
}
...
test("plugin renders valid comment tokens as spans", async () => {
  const tree = await runPlugin("revert @comment:a1b2c3d4e5 later", {
    fileExists: () => true,
  });
  const children = tree.children[0].children;
  assert.deepEqual(children.map((c) => c.type), ["text", "html", "text"]);
  assert.equal(children[1].value.includes("mention-token-comment"), true);
  assert.equal(children[1].value.includes('data-mention-value="a1b2c3d4e5"'), true);
});
