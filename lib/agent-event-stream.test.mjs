...
test("opens the transport before a slow session is ready and snapshots after subscribing", async () => {
  ...
  const replayedBufferedEvent = decodeData(await readWithin(reader));
  ...
  // The buffered message_update carries the same snapshot object the stream
  // replays next, so it is deduplicated; the buffered agent_start is not.
  assert.deepEqual(replayedBufferedEvent, { type: "agent_start" });
  assert.deepEqual(messageStart, { type: "message_start", message: snapshot });
...
test("skips buffered streaming events already reflected in the snapshot", async () => {
  const startup = deferred();
  const abortController = new AbortController();
  const stream = createAgentEventStream(
    new Request("http://localhost/events", { signal: abortController.signal }),
  ...
  const snapshot = { role: "assistant", content: [{ type: "text", text: "Hi" }] };
  startup.resolve({
    isStreaming: true,
    streamingMessage: snapshot,
    onEvent(nextListener) {
      // Same object reference as the snapshot: already included, skip it.
      nextListener({ type: "message_update", message: snapshot });
      nextListener({ type: "agent_start" });
      return () => {};
    },
  });
  ...
  assert.deepEqual(decodeData(await readWithin(reader)), { type: "agent_start" });
  assert.deepEqual(decodeData(await readWithin(reader)), {
    type: "message_start",
    message: snapshot,
  });
