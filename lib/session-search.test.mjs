...
test("ranks title matches first and finds renamed sessions without content hits", async (t) => {
  ...
  const byFirstMessage = fixture(t, [message("user", "unrelated")], { id: "fm", firstMessage: "Login refactor notes", modified: "2026-03-01" });

  const result = await searchSessionContents([contentHit, renamed, byFirstMessage], "login refactor");
  // Title hits sort before the newer content hit; the renamed session has no
  // content occurrence at all, so only a title search can find it.
  assert.deepEqual(result.results.map((r) => r.session.id), ["fm", "renamed", "content"]);
  assert.equal(result.results[0].titleHit, true);
  assert.equal(result.results[1].titleHit, true);
  assert.equal(result.results[1].entryId, undefined);
  assert.equal("titleHit" in result.results[2], false);
  assert.match(result.results[2].before, /discussing the /);
