...
test("firstMessage matches surface through the content scan, not as title hits", async (t) => {
  // firstMessage is message content: the content scan finds it in the file
  // and shows the snippet. Only the user-set name produces a title hit.
  const prefix = fixture(t, [message("user", "fix git settings now")], { id: "prefix", firstMessage: "fix git settings", modified: "2026-03-01" });
  const result = await searchSessionContents([prefix], "git");
  assert.deepEqual(result.results.map((r) => r.session.id), ["prefix"]);
  assert.equal("titleHit" in result.results[0], false);
  assert.match(result.results[0].before, /fix /);
});
