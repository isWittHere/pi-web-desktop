...
test("firstMessage is the rendered title when no name is set", async (t) => {
  // Unnamed sessions render firstMessage as their title, so a match anywhere
  // in it — even past the list's ellipsis — is a title hit. A session with a
  // name renders that name instead: its hidden firstMessage never counts.
  const unnamed = fixture(t, [message("user", "unrelated body")], { id: "unnamed", firstMessage: "please analyse the git history here", modified: "2026-03-01" });
  const deepPreview = fixture(t, [message("user", "unrelated body")], { id: "deep", firstMessage: "x".repeat(40) + " git tail", modified: "2026-04-01" });
  const namedOther = fixture(t, [message("user", "unrelated body")], { id: "namedOther", name: "周会记录", firstMessage: "login refactor notes with git", modified: "2026-05-01" });

  const result = await searchSessionContents([namedOther, deepPreview, unnamed], "git");
  assert.deepEqual(result.results.map((r) => r.session.id), ["deep", "unnamed"]);
  assert.equal(result.results[0].titleHit, true);
  assert.equal(result.results[1].titleHit, true);
  // namedOther's rendered title is "周会记录" — no "git", no title hit, and
  // its body never mentions it either, so it is not in the results at all.
});
