...
test("save/load workspace tabs round-trips through localStorage", () => {
  const store = new Map();
  globalThis.window = {
    localStorage: {
  ...
  try {
    const s = openWorkspace(openWorkspace(EMPTY_WORKSPACE_TABS, "a", "/a"), "b", "/b");
    saveWorkspaceTabs(s);
    assert.deepEqual(loadWorkspaceTabs(), s);
    // An empty tab list clears the key (falls back to single-tab seed).
    saveWorkspaceTabs(EMPTY_WORKSPACE_TABS);
    assert.equal(loadWorkspaceTabs(), null);
    // Corrupt JSON is ignored.
    store.set("pi-web:workspace-tabs", "{not json");
    assert.equal(loadWorkspaceTabs(), null);
    // Invalid entries are filtered out.
    store.set("pi-web:workspace-tabs", JSON.stringify({
      tabs: [{ key: "a", cwd: "/a" }, { junk: true }],
      activeKey: "a",
    }));
    assert.deepEqual(loadWorkspaceTabs(), { tabs: [{ key: "a", cwd: "/a" }], activeKey: "a" });
    // A missing/invalid activeKey falls back to the first tab.
    store.set("pi-web:workspace-tabs", JSON.stringify({
      tabs: [{ key: "a", cwd: "/a" }, { key: "b", cwd: "/b" }],
      activeKey: "nope",
    }));
    assert.equal(loadWorkspaceTabs().activeKey, "a");
  } finally {
