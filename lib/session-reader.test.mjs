...
test("readSessionPin returns the latest pin entry (latest wins)", () => {
  ...
  try {
    assert.equal(readSessionPin(filePath), true);
...
test("readSessionPin: trailing false entry clears an earlier pin", () => {
  ...
  try {
    assert.equal(readSessionPin(filePath), false);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
