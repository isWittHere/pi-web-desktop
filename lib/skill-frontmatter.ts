...
export function setDisableModelInvocation(content: string, disable: boolean): string {
  ...
  // document the key is never touched. Fence variants the SDK accepts (BOM,
  // trailing whitespace, CR-only endings, closing-fence suffixes) must survive
  // the edit — otherwise the skill becomes unloadable after toggling.
