...
test("isolated worktrees are unique, write-safe, and cleaned without deleting dirty work", async () => {
  ...
  try {
    ...
    // addWorktree creates a sibling `<repo>-worktrees` base directory; remove it
    // alongside the repo so successive isolation runs leave no leftover dirs.
    await rm(repo, { recursive: true, force: true });
    await rm(`${repo}-worktrees`, { recursive: true, force: true });
  }
});
