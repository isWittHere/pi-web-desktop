...
describe("escaped backticks in inline code", () => {
  it("rescales the marker so an escaped backtick is not a delimiter", () => {
    const escaped = "`" + "AudioManager" + String.raw`\`` + "1.cs" + "`";
    const out = normalizeDisplayMath(escaped);
    assert.ok(out.includes("AudioManager`1.cs"), "expected backtick to survive inside wider markers");
    assert.notEqual(out, "`AudioManager`1.cs`", "escaped backtick must not be treated as a plain delimiter");
  });
});
