/**
 * Global exponential backoff for web-password authentication failures.
 *
 * Pi Web serves a single operator and binds to 127.0.0.1, so route handlers
 * have no reliable client address (x-forwarded-for is spoofable and absent for
 * direct connections). Rather than trusting a per-IP key, every failure feeds
 * one shared counter and briefly blocks all password attempts. The only "other
 * user" affected by the block is the operator, and a bounded delay is
 * acceptable for them while it caps brute force at roughly one guess per
 * minute. Mirrors upstream 20ad98b (lib/auth-throttle.ts), wired into the
 * desktop proxy's Basic-auth path rather than a cookie login route.
 */
