...
// jiti resolves the subject's extensionless relative imports (git-graph-lanes)
// that plain node ESM cannot.
const jiti = createJiti(import.meta.url);

async function loadSubject() {
  return jiti.import("./comment-mentions.ts");
}
...
test("buildCommitLaneColors assigns stable lane indexes per branch line", async () => {
  const { buildCommitLaneColors } = await loadSubject();
  const commits = [
    { hash: "a".repeat(40), parents: ["p1"], subject: "tip of main" },
    { hash: "b".repeat(40), parents: ["p1"], subject: "tip of feature" },
    { hash: "p1", parents: [], subject: "shared root" },
  ];
  const colors = buildCommitLaneColors(commits);
  // Every commit gets an entry...
  assert.equal(colors.size, 3);
  // Linear first-parent history shares one lane color...
  assert.equal(colors.get("a".repeat(40)), colors.get("p1"));
  // ...while the side branch lands on a different lane color.
  assert.notEqual(colors.get("b".repeat(40)), colors.get("p1"));
});
