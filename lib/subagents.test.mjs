...
// Regression: the agents settings page rendered no nav badge because the
// shadowed sources were counted as separate entries.
test("badge counts only the profiles that win their scope", () => {
  ...
  assert.deepEqual(countEffectiveSubagentProfiles(profiles, true), { enabled: 2, total: 3 });
});

test("badge reports nothing enabled while the master switch is off", () => {
  const profiles = [
    { name: "explore", scope: "builtin", enabled: true },
    { name: "writer", scope: "global", enabled: true },
  ];
  assert.deepEqual(countEffectiveSubagentProfiles(profiles, false), { enabled: 0, total: 2 });
});
