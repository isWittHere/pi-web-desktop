...
const { isSubagentProfileOverridden, countEffectiveSubagentProfiles } = await createJiti(import.meta.url).import("./subagent-profile-precedence.ts");
...
// Regression: the agents settings page rendered no nav badge, then showed
// "0/3" while the master switch was off.
test("badge counts only the profiles that win their scope", () => {
  const profiles = [
    { name: "explore", scope: "builtin", enabled: true },
    { name: "explore", scope: "project", enabled: true },
    { name: "plan", scope: "builtin", enabled: true },
    { name: "writer", scope: "global", enabled: false },
  ];
  assert.equal(countEffectiveSubagentProfiles(profiles), 3);
});

test("badge counts disabled profiles too, matching the listed agents", () => {
  const profiles = [
    { name: "explore", scope: "builtin", enabled: true },
    { name: "writer", scope: "global", enabled: false },
  ];
  assert.equal(countEffectiveSubagentProfiles(profiles), 2);
  assert.equal(countEffectiveSubagentProfiles([]), 0);
});
...
test("a save keeps frontmatter keys this app does not manage", async () => {
  ...
  try {
    ...
    saveSubagentProfile(cwd, "project", profile({ name: "orchestrator", tools: ["read", "bash"] }));
...
test("a save refuses to overwrite malformed existing frontmatter", async () => {
  ...
  try {
    ...
    assert.throws(
      () => saveSubagentProfile(cwd, "project", profile({ name: "malformed" })),
...
test("the pi-subagents flag aliases are seeded, kept in step, and never overwrite a whitelist", async () => {
  ...
  try {
    ...
    saveSubagentProfile(cwd, "project", profile({ name: "fresh", loadSkills: true, loadExtensions: true }));
    ...
    saveSubagentProfile(cwd, "project", profile({ name: "fresh", loadSkills: false, loadExtensions: false }));
    ...
    saveSubagentProfile(cwd, "project", profile({ name: "scoped", loadExtensions: true }));
