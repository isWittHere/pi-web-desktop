...
import { invalidateModelsCache, loadModelsWithCache, withModelRuntimeError, withSafeModelLoadFailure } from "./models-cache.ts";
...
test("withSafeModelLoadFailure sets a fixed message without interpolating errors", () => {
  const data = modelsData("x");
  const failed = withSafeModelLoadFailure(data);
  assert.equal(typeof failed.modelError, "string");
  assert.match(failed.modelError, /temporarily unavailable/i);
  assert.ok(!failed.modelError.includes("C:\\"));
});

test("withModelRuntimeError only attaches a message when one exists", () => {
  const data = modelsData("x");
  assert.deepEqual(withModelRuntimeError(data, undefined), data);
  assert.equal(withModelRuntimeError(data, "bad provider").modelError, "bad provider");
});
