...
/**
 * Owns the EventSource, the agent-readiness handshake, and passive reconnect.
 *
 * The raw EventSource OPEN event only proves the HTTP transport is up — it
 * says nothing about whether the agent session finished starting. The server
 * therefore emits a `connected` data event only once the session is ready,
 * plus a `startup_error` event (closing the stream) when starting it failed.
 * Readiness (not transport openness) is what callers await on, with one
 * absolute deadline so a stalled cold start cannot hang the chat forever.
 *
 * Reconnects are passive: `maintain()` fires one `ensureConnected()` and lets
 * the failure path schedule a single retry later, so a transient network
 * blip re-attaches the stream while the session is still running. A call
 * that is no longer wanted (session switched, run settled) drops its retry
 * instead of reviving a stale connection.
 */
