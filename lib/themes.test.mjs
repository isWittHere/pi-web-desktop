...
test("lib/theme.ts normalizes search match tokens with pi 0.84.2 fallbacks", () => {
  const source = readFileSync(new URL("./theme.ts", import.meta.url), "utf8");

  // Parse-layer: the 0.84.2 search highlight tokens are collected even when
  // absent from a theme file.
  assert.match(source, /"searchMatchBg", "searchMatchText",/);

  // Resolution-layer: match pi's withThemeColorFallbacks so parsed themes
  // carry the same search-match semantics as the TUI.
  assert.match(source, /resolved\.searchMatchBg = resolved\.searchMatchBg \|\| resolved\.selectedBg \|\| ""/);
  assert.match(source, /resolved\.searchMatchText = resolved\.searchMatchText \|\| resolved\.text \|\| ""/);
});