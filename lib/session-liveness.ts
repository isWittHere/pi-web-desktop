...
// Shared Symbol.for key: extensions written against the upstream protocol
// register through the same global registry in this desktop fork.
    ...
    hasActiveProvider(session) {
      ...
      for (const provider of providers.values()) {
        ...
        try {
          ...
          // Fail safe: a broken probe must not cause the session to be reaped.
