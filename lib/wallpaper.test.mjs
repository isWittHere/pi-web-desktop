...
test("themeWallpaperPath maps built-in themes and falls back for unknown ones", () => {
  assert.equal(themeWallpaperPath(""), "/monet-artworks/default.jpg");
  assert.equal(themeWallpaperPath("gruvbox"), "/monet-artworks/gruvbox.jpg");
  assert.equal(themeWallpaperPath("miku-aqua"), "/monet-artworks/aqua.jpg");
  assert.equal(themeWallpaperPath("orbital-rose"), "/monet-artworks/rose.jpg");
  assert.equal(themeWallpaperPath("scarlet-tether"), "/monet-artworks/tether.jpg");
  assert.equal(themeWallpaperPath("solarized"), "/monet-artworks/solarized.jpg");
  assert.equal(themeWallpaperPath("custom-theme"), "/monet-artworks/default.jpg", "unknown falls back");
});

test("resolveWallpaperUrl prefers the user URL over the theme painting", () => {
  assert.equal(resolveWallpaperUrl("", "gruvbox"), "/monet-artworks/gruvbox.jpg");
  assert.equal(resolveWallpaperUrl("", ""), "/monet-artworks/default.jpg");
  assert.equal(resolveWallpaperUrl("data:image/png;base64,xxx", "gruvbox"), "data:image/png;base64,xxx");
});
