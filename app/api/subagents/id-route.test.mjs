...
// synthesize a Host header, so requests present the headers a real same-origin
// browser request would carry.
const LOCAL_HEADERS = {
  Host: "localhost",
...
function localGet(idOverride = id) {
  return new Request(`http://localhost/api/subagents/${idOverride}`, {
...
test("subagent route reads live state and accepts steer and abort actions", async (t) => {
  ...
  const getResponse = await GET(localGet(), context);
