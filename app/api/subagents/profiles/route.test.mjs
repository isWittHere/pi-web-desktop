...
// The route is guarded by the desktop request-security checks; undici does not
// synthesize a Host header, so every request below presents the headers a real
// same-origin browser request would carry.
const LOCAL_HEADERS = {
  Host: "localhost",
...
function localGet(url) {
  return new Request(url, { headers: { ...LOCAL_HEADERS } });
}
...
test("profiles route creates, lists, and deletes a project profile", async (t) => {
  ...
  const getResponse = await GET(localGet(`http://localhost/api/subagents/profiles?cwd=${encodeURIComponent(cwd)}`));
  ...
  const afterDelete = await GET(localGet(`http://localhost/api/subagents/profiles?cwd=${encodeURIComponent(cwd)}`));
...
test("profiles route keeps same-name global and project profiles independently editable", async (t) => {
  ...
  response = await GET(localGet(`http://localhost/api/subagents/profiles?cwd=${encodeURIComponent(cwd)}`));
...
test("profiles route rejects missing paths, malformed profiles, and unsafe names", async (t) => {
  ...
  let response = await GET(localGet("http://localhost/api/subagents/profiles"));
...
test("profiles route rejects cross-site requests via the desktop security guard", async () => {
  const crossSite = new Request("http://localhost/api/subagents/profiles?cwd=/tmp", {
    headers: { Host: "localhost", Origin: "http://evil.example", "Sec-Fetch-Site": "cross-site" },
  });
  const getResponse = await GET(crossSite);
  assert.equal(getResponse.status, 403);
  assert.deepEqual(await getResponse.json(), { error: "Untrusted API request" });

  const nonJson = new Request("http://localhost/api/subagents/profiles", {
    method: "PUT",
    headers: { ...LOCAL_HEADERS, "Content-Type": "text/plain" },
    body: "not json",
  });
  const putResponse = await PUT(nonJson);
  assert.equal(putResponse.status, 415);
  assert.deepEqual(await putResponse.json(), { error: "Content-Type must be application/json" });
});
