// Coverage for the context pagination API wiring: ?tail bounds the returned
// chain, ?before rewinds the walk and excludes its own boundary so prepending
// the page never duplicates it. Data behavior is covered end-to-end in
// lib/session-reader.pagination.test.mjs; here we assert the route wires the
// params through to buildSessionContext (excludeLeaf on ?before).
...
test("context route parses ?tail as opt-in and ?before with boundary exclusion", () => {
