  ...
  return (
    <div className="group/process relative mb-3 min-w-0">
      <div className="group/summary-row flex items-center justify-between">
        ...
        >
          <span className="truncate">{t("conversationCompacted")}</span>
      ...
      {expanded && (
        <div className="relative mt-2">
          <div ref={scrollRef} className="max-h-[280px] overflow-y-auto pr-2 text-text-dim">
            <p className="mb-2 text-xs leading-relaxed">{t("compactionSummaryDescription")}</p>
            {parsedSummary.body ? (
              <MarkdownBody className="!text-text-dim">{parsedSummary.body}</MarkdownBody>
            ) : (
              <span className="text-xs">{t("noSummary")}</span>
            )}
            {fileContext && (
              <div className="mt-3 border-t border-border pt-2 text-xs">
                <div className="font-medium">{t("fileContext").replace("{context}", fileContext)}</div>
                {parsedSummary.modifiedFiles.length > 0 && <FileList title={t("modifiedFiles")} files={parsedSummary.modifiedFiles} />}
                {parsedSummary.readFiles.length > 0 && <FileList title={t("readFiles")} files={parsedSummary.readFiles} />}
              </div>
            )}
          </div>
...
function getFileContext(
  ...
  modifiedFilesLabel: string,
): string | null {
  const parts: string[] = [];
  if (readFileCount > 0) parts.push(readFilesLabel.replace("{count}", String(readFileCount)));
  if (modifiedFileCount > 0) parts.push(modifiedFilesLabel.replace("{count}", String(modifiedFileCount)));
