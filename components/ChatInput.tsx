...
// Downscales oversized attachments before sending so a huge screenshot does
// not blow the request payload; GIFs keep their animation and small images
...
export async function compressImageFile(file: File): Promise<{ data: string; mimeType: string }> {
  ...
  try {
    ...
    // Keep the result only when it is meaningfully smaller than the original
    // (base64 inflates by ~4/3, so the compressed payload must beat that).
